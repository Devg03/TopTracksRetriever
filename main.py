...
def search_for_artist(token, artist_name):
    ...
    # Defines query. q = searching for what, & lets you type the type of content you are querying and limit sets the number of results
    query = f"?q={artist_name}&type=artist&limit = 1"

    query_url = url + query
    result = get(query_url, headers=headers)  # Stores the returned headers
    # Only searches for the artists name and related info.
...
artist = input(
    "Enter the name of the artist whose top tracks you wish to see: ")

result = search_for_artist(token, str(artist))  # Searchig for ACDC
artist_id = result["id"]  # stores the id of the artists
# stores the results of the query for top tracks
